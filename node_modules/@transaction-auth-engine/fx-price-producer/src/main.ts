...
import { createLogger, YahooFinanceClient } from '@transaction-auth-engine/shared';
...
function generateRealisticMovement(basePrice: number): number {
...
async function main(): Promise<void> {
  const logger = createLogger('fx-price-producer');
  const kafka = new Kafka({ clientId: 'fx-price-producer', brokers: KAFKA_BROKERS });
  ...
  const fxPrices = new Map<string, FxPrice>();
  ...
  const sendTicks = async (): Promise<void> => {
    try {
