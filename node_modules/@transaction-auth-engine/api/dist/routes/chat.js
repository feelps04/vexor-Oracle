...
function wsFromConnection(connection) {
    return (connection.socket ??
        connection);
}
async function requireWsAuth(app, req, ws) {
    const url = req?.url ?? '';
...
async function chatRoutes(app, opts) {
    ...
    app.get('/ws/chat', { websocket: true }, (connection, req) => {
        ...
        const join = async () => {
            ...
            if (!auth)
                return;
            const memberRes = await pg.query(`SELECT 1 FROM chat_members WHERE room_id = $1 AND user_id = $2`, [roomId, auth.userId]);
    ...
    app.get('/api/v1/chat/rooms', async (request, reply) => {
        const user = await (0, auth_js_1.requireAuth)(app, request, reply);
        if (!user)
            return;
        const res = await pg.query(`SELECT r.id, r.type, r.name, r.created_at
       FROM chat_rooms r
       JOIN chat_members m ON m.room_id = r.id
       WHERE m.user_id = $1
       ORDER BY r.created_at DESC`, [user.userId]);
        return reply.send({ rooms: res.rows });
    });
    app.post('/api/v1/chat/rooms', async (request, reply) => {
        const user = await (0, auth_js_1.requireAuth)(app, request, reply);
        if (!user)
            return;
        ...
        try {
            ...
            const roomRes = await client.query(`INSERT INTO chat_rooms(type, name, created_by)
           VALUES ('room', $1, $2)
           RETURNING id`, [name, user.userId]);
            const roomId = roomRes.rows[0].id;
            await client.query(`INSERT INTO chat_members(room_id, user_id, role)
           VALUES ($1, $2, 'admin')
           ON CONFLICT DO NOTHING`, [roomId, user.userId]);
            if (memberEmails.length) {
                const usersRes = await client.query(`SELECT id FROM users WHERE email = ANY($1::text[])`, [memberEmails.map((e) => String(e).trim().toLowerCase())]);
                for (const row of usersRes.rows) {
                    await client.query(`INSERT INTO chat_members(room_id, user_id, role)
               VALUES ($1, $2, 'member')
               ON CONFLICT DO NOTHING`, [roomId, row.id]);
                }
            }
    ...
    app.post('/api/v1/chat/dm', async (request, reply) => {
        const user = await (0, auth_js_1.requireAuth)(app, request, reply);
        if (!user)
            return;
        ...
        const otherRes = await pg.query('SELECT id FROM users WHERE email = $1', [otherEmail]);
        ...
        const dmRes = await pg.query(`SELECT r.id
         FROM chat_rooms r
         JOIN chat_members m1 ON m1.room_id = r.id AND m1.user_id = $1
         JOIN chat_members m2 ON m2.room_id = r.id AND m2.user_id = $2
         WHERE r.type = 'dm'
         LIMIT 1`, [user.userId, otherUserId]);
        if ((dmRes.rowCount ?? 0) > 0)
            return reply.send({ roomId: dmRes.rows[0].id });
        const client = await pg.connect();
        try {
            await client.query('BEGIN');
            const roomRes = await client.query(`INSERT INTO chat_rooms(type, name, created_by)
           ...
           ON CONFLICT DO NOTHING`, [roomId, user.userId, otherUserId]);
            await client.query('COMMIT');
            return reply.code(201).send({ roomId });
        }
        catch (err) {
    ...
    app.get('/api/v1/chat/rooms/:id/messages', async (request, reply) => {
        const user = await (0, auth_js_1.requireAuth)(app, request, reply);
        if (!user)
            return;
        const roomId = request.params.id;
        const memberRes = await pg.query(`SELECT 1 FROM chat_members WHERE room_id = $1 AND user_id = $2`, [roomId, user.userId]);
        if (memberRes.rowCount === 0)
            return reply.code(403).send({ message: 'Forbidden' });
        const res = await pg.query(`SELECT id, room_id, sender_user_id, content, created_at
         FROM chat_messages
         WHERE room_id = $1
    ...
    app.post('/api/v1/chat/rooms/:id/messages', async (request, reply) => {
        const user = await (0, auth_js_1.requireAuth)(app, request, reply);
        if (!user)
            return;
        const roomId = request.params.id;
        ...
        if (!content)
            return reply.code(400).send({ message: 'Mensagem vazia' });
        const memberRes = await pg.query(`SELECT 1 FROM chat_members WHERE room_id = $1 AND user_id = $2`, [roomId, user.userId]);
        if (memberRes.rowCount === 0)
            return reply.code(403).send({ message: 'Forbidden' });
        const res = await pg.query(`INSERT INTO chat_messages(room_id, sender_user_id, content)
         VALUES ($1, $2, $3)
         RETURNING id`, [roomId, user.userId, content]);
        const msgRes = await pg.query(`SELECT id, room_id, sender_user_id, content, created_at
         FROM chat_messages
