...
function parseTtlSec(envValue, fallback) {
...
async function authRoutes(app, opts) {
    ...
    app.post('/api/v1/auth/register', {
        ...
        try {
            ...
            const userRes = await client.query('INSERT INTO users(email, password_hash) VALUES ($1, $2) RETURNING id', [email, passwordHash]);
            const userId = userRes.rows[0].id;
            const accountRes = await client.query('INSERT INTO accounts(user_id) VALUES ($1) RETURNING id', [userId]);
            ...
            await client.query('INSERT INTO refresh_tokens(user_id, token_hash, expires_at) VALUES ($1, $2, $3)', [userId, refreshHash, expiresAt.toISOString()]);
            await client.query('COMMIT');
            const accessToken = app.jwt.sign({ email, accountId }, { sub: userId, expiresIn: accessTtlSec });
    ...
    app.post('/api/v1/auth/login', {
        ...
        try {
            ...
            const accountRes = await pg.query('SELECT id FROM accounts WHERE user_id = $1', [userId]);
            ...
            const refreshHash = sha256Base64(refreshToken);
            const expiresAt = new Date(Date.now() + refreshTtlSec * 1000);
            await pg.query('INSERT INTO refresh_tokens(user_id, token_hash, expires_at) VALUES ($1, $2, $3)', [userId, refreshHash, expiresAt.toISOString()]);
            const accessToken = app.jwt.sign({ email, accountId }, { sub: userId, expiresIn: accessTtlSec });
            reply.setCookie('refreshToken', refreshToken, {
                httpOnly: true,
                secure: cookieSecure(),
            ...
            const isDbUnavailable = msg.toLowerCase().includes('connection terminated') ||
