...
const submit_btc_order_js_1 = require("../use-cases/submit-btc-order.js");
const submit_stock_order_js_1 = require("../use-cases/submit-stock-order.js");
...
async function applyEmotionalScore(redis, accountId) {
    ...
    try {
        ...
        stress = raw ? JSON.parse(raw) : null;
    }
    catch {
        stress = null;
    }
...
async function orderRoutes(app, opts) {
    ...
    app.post('/api/v1/orders/btc', {
        ...
        if (redis) {
            try {
                ...
                if (scored && scored.delta !== 0) {
                    request.log?.info?.({ teamId: scored.teamId, delta: scored.delta, level: scored.level }, 'Team emotional score updated (btc order)');
                }
            }
            catch {
        ...
        const transaction = await (0, submit_btc_order_js_1.submitBtcOrder)(producer, mercadoBitcoin, {
        ...
        try {
            if (redis) {
                try {
                    const scored = await applyEmotionalScore(redis, body.accountId);
                    if (scored && scored.delta !== 0) {
                        request.log?.info?.({ teamId: scored.teamId, delta: scored.delta, level: scored.level }, 'Team emotional score updated (stock order)');
                    }
                }
                catch {
                    // ignore scoring failures
            ...
            const transaction = await (0, submit_stock_order_js_1.submitStockOrder)(producer, brapi, {
                accountId: body.accountId,
                symbol: body.symbol,
