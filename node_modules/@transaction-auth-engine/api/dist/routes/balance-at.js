...
exports.balanceAtRoutes = balanceAtRoutes;
const INITIAL_BALANCE = 0;
async function balanceAtRoutes(app, opts) {
    const { pg } = opts;
    app.get('/api/v1/accounts/:accountId/balance-at', {
        schema: {
        ...
        let netOperations = 0;
        try {
            const depositsRes = await pg.query(`SELECT COALESCE(SUM(
            CASE 
           ...
           WHERE account_id = $1 AND created_at <= $2`, [accountId, atDate.toISOString()]);
    ...
    app.get('/api/v1/accounts/:accountId/balance-series', {
        schema: {
            params: { type: 'object', properties: { accountId: { type: 'string' } }, required: ['accountId'] },
            response: {
                200: {
        ...
        let rows = [];
        try {
            const res = await pg.query(`SELECT created_at, type, amount
           FROM balance_operations
           WHERE account_id = $1
