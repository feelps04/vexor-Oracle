...
import { createLogger, MercadoBitcoinClient } from '@transaction-auth-engine/shared';
...
async function main(): Promise<void> {
  const logger = createLogger('btc-price-producer');
  ...
  const sendPrice = async (): Promise<void> => {
    try {
      let priceBRL: number;
      try {
