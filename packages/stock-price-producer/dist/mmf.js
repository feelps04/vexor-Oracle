...
exports.openMmfWithConfig = openMmfWithConfig;
exports.openMmf = openMmf;
exports.closeMmf = closeMmf;
exports.readAllRecordsWithConfig = readAllRecordsWithConfig;
...
const voidPtr = ref_napi_1.default.refType(ref_napi_1.default.types.void);
// Win32 constants
...
function assertConfig(cfg) {
    if (!cfg.name)
        throw new Error('MMF name is required');
...
// kernel32 bindings
const kernel32 = ffi_napi_1.default.Library('kernel32', {
...
function toWideString(str) {
    const buf = node_buffer_1.Buffer.from(str + '\0', 'utf16le');
    return buf;
}
function buildMmfNameCandidates(name) {
...
function openMmfWithConfig(cfg) {
    assertConfig(cfg);
...
function closeMmf(mmf) {
    if (!mmf)
        return;
    try {
        if (mmf.view && !ref_napi_1.default.isNull(mmf.view)) {
    ...
    try {
        if (mmf.handle && !ref_napi_1.default.isNull(mmf.handle)) {
...
function decodeSymbol(buf, offset, symbolBytes) {
...
function readAllRecordsWithConfig(view, cfg) {
    assertConfig(cfg);
    const out = [];
