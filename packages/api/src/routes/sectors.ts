...
export async function sectorRoutes(app: FastifyInstance, opts?: { redis?: Redis }): Promise<void> {
  ...
  let indexCache: SectorIndex | null = null;
  const getIndex = (): SectorIndex => {
  ...
  const fetchBinanceQuotes = async (symbols: string[]): Promise<Map<string, number>> => {
    const out = new Map<string, number>();
    try {
      ...
      for (const sym of symbols) {
        try {
          ...
          const data: any = await res.json();
          if (data.price) out.set(sym, parseFloat(data.price));
        } catch {}
      }
    }
    return out;
