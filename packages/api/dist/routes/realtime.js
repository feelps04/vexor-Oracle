...
async function getRedisBalances(redis, accounts) {
    const out = {};
...
async function getSqlBalances(pg, accounts) {
    const out = {};
...
async function getTimeMachineBalance(pg, accountId, atIso) {
    const atDate = new Date(atIso);
    if (Number.isNaN(atDate.getTime()))
        return INITIAL_BALANCE;
    try {
        const res = await pg.query(`SELECT COALESCE(SUM(
        CASE
          WHEN type = 'deposit' THEN amount
          WHEN type = 'withdraw' THEN -amount
          ELSE 0
...
async function realtimeRoutes(app, opts) {
    const { redis, pg } = opts;
