...
let pg;
async function socialRoutes(app, opts) {
    pg = opts?.pg;
    // ============ USERS ============
    ...
    app.get('/api/v1/social/users/:userId', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
    ...
    app.get('/api/v1/social/users', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { q } = req.query;
        const result = await pg.query(`SELECT * FROM social_users WHERE username ILIKE $1 OR display_name ILIKE $1 LIMIT 20`, [`%${q || ''}%`]);
        return { users: result.rows };
    });
    // ============ FEED & POSTS ============
    app.get('/api/v1/social/feed', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { page = 0, limit = 20 } = req.query;
        const offset = Number(page) * Number(limit);
        const postsResult = await pg.query(`SELECT p.*, u.id as user_id, u.username, u.display_name, u.avatar, u.verified, u.is_ai,
       ...
       LIMIT $2 OFFSET $3`, ['a0000000-0000-0000-0000-000000000001', Number(limit), offset]);
    ...
    app.post('/api/v1/social/posts', async (req, reply) => {
        ...
        const result = await pg.query(`INSERT INTO social_posts (user_id, content, type, media, tags) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [userId, content, type, JSON.stringify(media), tags]);
        return { post: result.rows[0] };
    });
    app.post('/api/v1/social/posts/:postId/like', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
    ...
    app.post('/api/v1/social/posts/:postId/comments', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { postId } = req.params;
    ...
    app.get('/api/v1/social/posts/:postId/comments', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { postId } = req.params;
        const result = await pg.query(`SELECT c.*, u.id as user_id, u.username, u.display_name, u.avatar, u.verified FROM social_comments c JOIN social_users u ON c.user_id = u.id WHERE c.post_id = $1 ORDER BY c.created_at ASC`, [postId]);
    ...
    app.get('/api/v1/social/stories', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const result = await pg.query(`SELECT s.*, u.id as user_id, u.username, u.display_name, u.avatar FROM social_stories s JOIN social_users u ON s.user_id = u.id WHERE s.expires_at > NOW() ORDER BY s.created_at DESC`);
    ...
    app.post('/api/v1/social/stories', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
    ...
    // ============ REELS ============
    app.get('/api/v1/social/reels', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const result = await pg.query(`SELECT p.*, u.id as user_id, u.username, u.display_name, u.avatar, u.verified, (SELECT COUNT(*) FROM social_likes WHERE post_id = p.id) as likes_count FROM social_posts p JOIN social_users u ON p.user_id = u.id WHERE p.type = 'reel' ORDER BY p.created_at DESC`);
    ...
    app.get('/api/v1/social/conversations', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const userId = 'a0000000-0000-0000-0000-000000000001';
        const result = await pg.query(`SELECT DISTINCT CASE WHEN m.from_user_id = $1 THEN m.to_user_id ELSE m.from_user_id END as other_user_id FROM social_messages m WHERE m.from_user_id = $1 OR m.to_user_id = $1`, [userId]);
        const conversations = await Promise.all(result.rows.map(async (row) => {
            ...
            const lastMsg = await pg.query(`SELECT content, created_at FROM social_messages WHERE (from_user_id = $1 AND to_user_id = $2) OR (from_user_id = $2 AND to_user_id = $1) ORDER BY created_at DESC LIMIT 1`, [userId, row.other_user_id]);
    ...
    app.get('/api/v1/social/messages/:otherUserId', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
    ...
    app.post('/api/v1/social/messages', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
    ...
    app.get('/api/v1/social/squads', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const userId = 'a0000000-0000-0000-0000-000000000001';
        const result = await pg.query(`SELECT s.*, (SELECT COUNT(*) FROM social_squad_members WHERE squad_id = s.id) as member_count FROM social_squads s JOIN social_squad_members sm ON s.id = sm.squad_id WHERE sm.user_id = $1`, [userId]);
        return { squads: result.rows };
    });
    app.get('/api/v1/social/squads/:squadId', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { squadId } = req.params;
        const result = await pg.query(`SELECT s.*, (SELECT json_agg(json_build_object('id', u.id, 'username', u.username, 'displayName', u.display_name, 'avatar', u.avatar)) FROM social_squad_members sm JOIN social_users u ON sm.user_id = u.id WHERE sm.squad_id = s.id) as members FROM social_squads s WHERE s.id = $1`, [squadId]);
        if (result.rows.length === 0)
            return reply.code(404).send({ error: 'Squad not found' });
        return { squad: result.rows[0] };
    });
    app.post('/api/v1/social/squads', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
    ...
    app.get('/api/v1/social/squads/:squadId/messages', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { squadId } = req.params;
        const result = await pg.query(`SELECT m.*, u.id as user_id, u.username, u.display_name, u.avatar, u.verified FROM social_squad_messages m JOIN social_users u ON m.user_id = u.id WHERE m.squad_id = $1 ORDER BY m.created_at ASC LIMIT 100`, [squadId]);
    ...
    app.post('/api/v1/social/squads/:squadId/messages', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { squadId } = req.params;
        ...
        if (memberCheck.rows.length === 0)
            return reply.code(403).send({ error: 'Not a member of this squad' });
    ...
    app.post('/api/v1/social/squads/:squadId/join', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { squadId } = req.params;
        const userId = 'a0000000-0000-0000-0000-000000000001';
        try {
            ...
            return reply.code(400).send({ error: 'Already a member or squad not found' });
        }
    });
    // ============ FOLLOW ============
    app.post('/api/v1/social/users/:userId/follow', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { userId } = req.params;
    ...
    app.get('/api/v1/social/users/:userId/followers', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const { userId } = req.params;
    ...
    app.get('/api/v1/social/notifications', async (req, reply) => {
        if (!pg)
            return reply.code(500).send({ error: 'Database not available' });
        const userId = 'a0000000-0000-0000-0000-000000000001';
        const result = await pg.query(`SELECT n.*, u.id as from_user_id, u.username, u.display_name, u.avatar FROM social_notifications n LEFT JOIN social_users u ON n.from_user_id = u.id WHERE n.user_id = $1 ORDER BY n.created_at DESC LIMIT 50`, [userId]);
