...
async function teamsRoutes(app, opts) {
    const { redis } = opts;
    app.post('/api/v1/teams/join', async (req, reply) => {
        const accountId = String(req.body?.accountId ?? '').trim();
        const teamId = String(req.body?.teamId ?? '').trim();
        if (!accountId)
            return reply.status(400).send({ message: 'accountId is required' });
        if (!teamId)
    ...
    app.get('/api/v1/accounts/:accountId/team', async (req, reply) => {
    ...
    app.get('/api/v1/teams/:teamId/score', async (req, reply) => {
