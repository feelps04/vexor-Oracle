...
async function broadcastTicks(ticks) {
    const message = { type: 'ticks', items: ticks };
...
async function sendInitialPrices(session, symbols) {
    const lastPrices = {};
...
async function handleClientMessage(session, data) {
    try {
        ...
        if (msg.type === 'set_symbols' && Array.isArray(msg.symbols)) {
            const newSymbols = new Set(msg.symbols.map((s) => s.toUpperCase()));
        ...
        if (msg.type === 'get_sector' && msg.sector_id) {
            ...
            sessions.set(session, new Set(symbols));
